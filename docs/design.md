...
扩展创建完全独立的常驻入口，不把入口注入聊天文件管理面板、欢迎页或其他原生弹窗：

- 桌面端：在顶栏提供独立的归档图标按钮
- 移动端：提供不遮挡输入区的独立浮动按钮
- 命令入口：注册 `/chat-browser` 作为键盘和故障回退入口

入口在当前是否打开聊天、是否位于欢迎页两种状态下都可使用。其可见性和位置由扩展自身管理，不依赖原生聊天文件面板是否已经创建 DOM。

浏览器使用独立弹窗或侧边抽屉，不复用欢迎页 DOM，也不模拟点击首页卡片。关闭浏览器不会切换或重载当前聊天。

原生聊天文件面板可以在后续阶段增加“分割当前记录”等轻量快捷操作，但这些增强是可选适配层；适配层失效时，独立入口和完整浏览器必须继续正常工作。
...
独立常驻按钮打开状态面板，扩展设置中只提供备用入口。状态面板显示：
...
2. 全量重建后能展示当前账号全部有效单聊和群聊
3. 角色头像文件名和群组 ID 能正确映射为可读名称
4. 点击记录能用原生函数打开精确聊天，而不是只打开角色最后一次聊天
5. 数千条索引不会一次性生成数千个 DOM 节点
6. 普通打开和筛选不重复扫描全部 JSONL
7. 创建、重命名、删除和当前聊天更新后缓存能正确失效
8. 孤立聊天和接口失败不会导致旧的完整索引被静默清空
9. 过大聊天可以从浏览器直接进入同一个无损分割向导
