...
- 扩展形态：纯前端第三方扩展
- 暂定名称：SillyTavern Chat Manager（聊天管理）
- 设计原则：不修改 SillyTavern 源码，不拦截原生保存流程，不在后台重命名、删除或覆盖现有聊天与备份
- 核心目标：可靠识别备份归属；安全分割过大的单聊和群聊记录；随时浏览当前账号的全部聊天
...
9. 在任意酒馆页面打开全局聊天记录浏览器，不依赖欢迎页或页面刷新
10. 统一展示单聊、群聊和根级聊天，并支持筛选、排序；归属有效的聊天可直接打开并进入分割流程
11. 在原生聊天文件列表的每条记录中提供“相关备份”和“分割”快捷操作
...
6. 不在后台持续扫描全部聊天内容，也不建立服务端常驻索引
7. 不声称覆盖或优化 SillyTavern 核心；缓存、流式读取和分页仅是插件自身的实现约束
...
流式读取、IndexedDB 缓存、分页和幂等 DOM 注入只用于保证插件自身稳定，不宣传为对 SillyTavern 的功能或性能覆盖。“备份识别”是管理功能，也不称为性能优化。
...
欢迎页通过 `POST /api/chats/recent` 获取最近聊天。该接口会枚举当前账号的角色聊天目录、群聊登记列表和根级聊天；`max` 只限制最终返回数量，省略 `max` 时会返回全部记录。

因此“只能刷新首页后查看”是现有前端入口限制，不是服务端接口限制。扩展可以在任意页面调用同一接口并提供独立浏览器，无需修改欢迎页。
...
| `/api/chats/recent` | POST | 跨角色、群组和根目录枚举聊天；省略 `max` 可返回全部 | 否 |
| `/api/chats/search` | POST | 对指定角色或群组执行聊天名及内容搜索 | 否 |
...
| 欢迎页最近聊天的获取、展示与打开方式 | [`SillyTavern/public/scripts/welcome-screen.js`](../../SillyTavern/public/scripts/welcome-screen.js) |
...
聊天面板直接展示原生接口返回的文件大小和消息数量，不增加独立检测器，也不统计 Swipe 字节、最大单条消息或页面 DOM 数量。

设置中只提供一个“文件大小警告值（MiB）”，默认 16 MiB。文件达到该值时：

- 在聊天行显示警告颜色或图标
- 允许在全局聊天列表中筛选“超过警告值”

警告不自动分割聊天，不改变排序，也不代表酒馆存在错误。用户始终可以从面板直接看到原始文件大小和消息数量。
...
分割使用 SillyTavern 原生消息 ID，即页面消息元素的 `mesid` 和开启“Message IDs”后显示的 `#编号`：

- 第一条消息是 `#0`，对应消息数组 `chat[0]`
- 最后一条消息是 `#(N - 1)`
- JSONL 第一行聊天头不属于消息楼层
- 系统消息、隐藏消息如果存在于消息数组中，仍占用其原生 `mesid`
- 一个楼层无论包含多少 Swipe 都只计为一个楼层；Swipe 数据随该楼层完整保留

输入框和预览统一显示 `#mesid`，不额外引入从 1 开始的第二套编号。

### 8.3 分割模式

#### 指定范围

用户填写起始楼层 `#S` 和结束楼层 `#E`，两端都包含，并满足：
...
- 提取范围：只生成包含 `#S..#E` 的新聊天
- 完整切分：生成 `#0..#(S-1)`、`#S..#E`、`#(E+1)..#(N-1)`，空范围不生成文件

#### 按固定楼层数

用户填写每卷楼层数 `X`，默认值为 500。默认处理 `#0..#(N-1)`，也可以先指定 `#S..#E`，再只对该范围按每 `X` 层分卷。

示例：范围 `#100..#349`，每卷 100 层，输出：
...
最后不足 `X` 层的余数单独成为最后一卷，不丢弃也不并入前一卷。

### 8.4 边界与内容规则

1. 所有起止楼层均按包含关系处理
2. 只在消息对象之间切分，不拆开单条消息
3. 不修改消息对象，不在分卷之间添加重复上下文消息
4. 不插入 AI 总结；总结不属于无损分割
5. 文件字节大小仅用于预览和超大分卷警告，不覆盖用户指定的楼层边界
6. 单条消息本身很大时仍完整保留在所属楼层，不按字节截断

选择“完整切分”或对完整聊天执行固定量分割时，将所有分卷去掉聊天头后按序拼接，应与原消息序列完全一致。选择“提取范围”时，只验证所选楼层序列一致。
...
```json
{
  "chat_metadata": {
    ...
    "chat_manager": {
      ...
      "coverageMode": "full-or-selection",
      "selectionStart": 0,
      "selectionEnd": 499,
      "selectionDigest": "所选楼层序列 SHA-256",
      "partIndex": 1,
      "partCount": 4,
      "messageStart": 0,
      "messageEnd": 499,
...
5. `coverageMode` 为 `full` 时，将全部分卷消息按原顺序重新计算出的摘要等于 `sourceDigest`
6. `coverageMode` 为 `selection` 时，将全部分卷消息按原顺序重新计算出的摘要等于 `selectionDigest`
...
扩展只提供一个统一界面入口。原生“聊天文件”菜单项是 `#option_select_chat`；扩展在其后插入同级菜单项：
...
入口通过 `#option_select_chat.insertAdjacentElement('afterend', element)` 定位，因此桌面端和移动端共用酒馆同一聊天选项菜单，不做两套入口，也不把按钮插进已打开的聊天文件列表内容中。

初始化必须先检查 `#chat_manager_open` 是否已经存在，避免扩展重新激活或界面重建时重复插入。若 `#option_select_chat` 不存在，则记录明确的版本兼容错误，不尝试猜测其他插入位置。

浏览器使用独立弹窗或侧边抽屉，不复用欢迎页 DOM，也不模拟点击首页卡片。关闭浏览器不会切换或重载当前聊天。

### 9.2 数据获取

分为三种读取层级：

1. 打开浏览器：立即展示 IndexedDB 中的上次索引
2. 日常刷新：调用 `/api/chats/recent`，默认只请求最近 100 条并与本地索引合并
3. 重建全部索引：用户明确点击后调用 `/api/chats/recent` 且省略 `max`，获取当前账号全部聊天

全量接口返回的数据包括实际文件名、头像文件名或群组 ID、文件大小、消息数、末条消息和末条消息时间。前端再通过 `characters` 和 `groups` 补充可读名称与缩略图。

索引只保存末条消息的截断预览，不保存接口返回的无限长度原文，避免少数超长消息放大 IndexedDB 和列表渲染开销。

禁止每次打开浏览器都请求全部聊天。`/api/chats/recent` 的全量模式会在服务端枚举全部文件，并为返回项读取 JSONL 以统计消息和末条记录；大型档案库必须由用户主动触发全量重建。
...
- 按角色、群组、聊天类型和是否超过警告值筛选
- 按最后活动时间、文件大小、消息数和名称排序
- 对已加载索引搜索角色名、群组名、聊天名和末条消息预览
- 仅显示超过警告值的聊天
- 在单一角色或群组范围内调用 `/api/chats/search` 做显式深度内容搜索

全局深度内容搜索默认关闭，因为原生搜索接口按角色或群组工作，跨全部角色扇出请求会重复扫描大量文件。

### 9.4 打开聊天

打开单聊时：

1. 用记录中的 `avatar` 在 `characters` 中寻找角色
2. 调用 `selectCharacterById()` 选择角色
3. 从 `file_name` 去掉 `.jsonl`
4. 调用 `openCharacterChat(chatName)` 打开目标聊天

打开群聊时，从原生 `group-chats.js` 导入并调用 `openGroupById(groupId)`，然后调用 `openGroupChat(groupId, chatName)`。

打开前检查是否正在生成或流式输出；忙碌时禁止切换。目标角色、群组已删除或记录属于根级聊天时，记录显示为孤立聊天，不提供直接打开，但可以保留在索引中供诊断。只有能通过原生接口读取完整内容的归属记录才提供本地导出或分割。
...
“制定分割计划”只把精确的头像文件名或群组 ID、聊天文件名和当前统计交给分割模块，不直接执行写入。用户仍需经过分割预览和二次确认。

### 9.6 索引一致性

监听聊天创建、重命名、删除和消息变化事件：

- 当前聊天发生消息变化时，只更新当前聊天的内存统计并把该条标记为待刷新
- 聊天创建、重命名或删除时，使相关所有者的缓存失效
- 群聊创建或删除时，使相关群组缓存失效
- 不因单条消息事件触发全量 `/api/chats/recent`

界面必须显示索引更新时间，并提供“刷新最近记录”和“重建全部索引”两个不同按钮。缓存记录不再存在于最新结果时，先标记为可能已删除；只有全量重建后才从索引移除。

### 9.7 分页与渲染性能

即使索引包含数千条聊天，DOM 中也只渲染当前页或可视窗口。第一版采用每页 50 条的客户端分页；后续可在不改变数据层的前提下升级为虚拟列表。

缩略图使用懒加载。排序和普通筛选只处理本地索引，不重新请求服务器。

### 9.8 原生聊天文件列表增强

原生聊天文件面板继续负责当前角色或当前群组的聊天列表。扩展不替换其数据请求、模板或现有操作，只在每条记录的 `.select_chat_actions` 末尾添加两个快捷按钮：

```text
导出 JSONL　导出 TXT　删除　相关备份　分割
```

注入位置和身份读取：

- 列表容器：`#select_chat_div`
- 行容器：`.select_chat_block_wrapper`
- 聊天标识：`.select_chat_block[file_name]` 的 `file_name` 属性
- 操作容器：`.select_chat_actions`
- 当前所有者：`SillyTavern.getContext()` 的 `characterId`、`groupId`、`characters` 和 `groups`

`native-chat-panel.js` 观察 `#select_chat_div` 的子节点变化，因为原生 `displayPastChats()` 每次打开或刷新面板时会清空并重新创建列表。每行使用 `data-chat-archive-enhanced="true"` 标记，保证重复渲染和扩展重新激活时不会重复插入按钮。

按钮行为：

- “相关备份”：以当前所有者和精确聊天文件名启动备份识别，只展示 `verified`、用户确认绑定或仍需确认的候选备份
- “分割”：读取该行对应的完整聊天，进入同一个无损分割计划和确认向导

两个按钮都使用事件委托绑定在 `#select_chat_div`，不覆盖原生点击处理。点击按钮必须阻止事件冒泡，避免同时触发“打开聊天”。

若原生选择器不存在，扩展记录版本兼容错误并停止列表增强；全局“聊天管理”入口、备份识别器和分割器仍可使用。
...
type ChatIndexEntry = {
    key: string;               // ownerType + ownerId + file_name
    ownerType: 'character' | 'group' | 'root';
    ownerId?: string;          // 头像文件名或群组 ID
    ownerDisplayName?: string;
    fileName: string;
    fileSize: string;
    messageCount: number;
    lastMessageAt: string;
    preview: string;           // 截断后的末条消息
    indexedAt: string;
    stale: boolean;
};
```

`backup-index` 与 `chat-index` 使用独立 object store。服务器列表中的文件大小或末条消息时间变化后，缓存自动失效。用户可以分别清除备份识别缓存和聊天索引；清理缓存不调用任何服务器删除接口。
...
2. 查看归属证据
3. 选择“指定范围”或“固定楼层数”
4. 填写 `#S..#E`；固定量模式再填写每卷楼层数 `X`
5. 范围模式选择“仅提取范围”或“完整切分”
6. 预览每卷楼层范围、楼层数、文件大小和名称
7. 选择本地导出或安装为酒馆聊天
8. 二次确认
9. 显示逐卷写入与校验进度
10. 输出完成报告
...
| `chat-browser.js` | 全局聊天索引、筛选、排序、打开与事件失效处理 |
...
| `native-chat-panel.js` | 向原生聊天文件行幂等注入备份和分割快捷操作 |
| `ui.js` | 设置、识别列表和分割向导 |
...
| 全量聊天索引失败 | 保留旧缓存并标记为过期，不用不完整结果覆盖完整索引 |
| 角色或群组已删除 | 标记为孤立记录，禁止直接打开 |
| 聊天切换时正在生成 | 阻止切换并保留浏览器当前筛选状态 |
| 原生聊天文件面板选择器变化 | 禁用行级增强并报告兼容问题，不影响独立“聊天管理”入口 |
...
3. 完整覆盖模式下，拼接全部分卷后与原消息序列的规范化摘要一致
4. 每卷拥有独立 `integrity`
5. 原聊天在成功、失败和取消情况下均保持不变
6. 目标名已存在时绝不覆盖
7. 中途保存失败后不会继续写入，且能精确报告部分输出
8. 大文件使用流式解析，识别时不会因一次性 `response.text()` 产生额外完整副本
9. 指定范围 `#S..#E` 时首尾楼层均包含，且不会出现范围外消息
10. 固定量分割时每卷最多 `X` 层，最后余数卷完整保留
11. 对限定范围执行固定量分割时不会越过 `#S` 或 `#E`

### 15.3 全局聊天浏览器

1. 不刷新页面、不返回欢迎页也能打开浏览器
2. 唯一入口紧跟在原生 `#option_select_chat` 后面，重复初始化不会产生重复菜单项
3. 全量重建后能展示当前账号全部有效单聊和群聊
4. 角色头像文件名和群组 ID 能正确映射为可读名称
5. 点击记录能用原生函数打开精确聊天，而不是只打开角色最后一次聊天
6. 数千条索引不会一次性生成数千个 DOM 节点
7. 普通打开和筛选不重复扫描全部 JSONL
8. 创建、重命名、删除和当前聊天更新后缓存能正确失效
9. 孤立聊天和接口失败不会导致旧的完整索引被静默清空
10. 任意归属有效的聊天可以从浏览器直接进入同一个无损分割向导

### 15.4 原生聊天文件列表增强

1. 每条原生聊天记录最多出现一个“相关备份”和一个“分割”按钮
2. 重新打开或刷新聊天文件面板不会重复插入按钮
3. 行级按钮能取得当前角色或群组及精确聊天文件名
4. “相关备份”不会把同下划线前缀但证据不匹配的备份自动归属
5. “分割”打开统一向导，不直接写入或删除文件
6. 点击扩展按钮不会同时触发打开聊天、导出或删除等原生行为
7. 原生选择器不兼容时，全局面板仍能正常工作
...
- 分割过程中切换聊天或开始生成
- 从普通聊天页面随时打开全局浏览器
- 使用 `/api/chats/recent` 的最近模式与全量模式
- 从浏览器精确打开单聊和群聊
- 已删除角色、已删除群组和根级孤立聊天
- 当前角色和群组聊天文件列表中的备份、分割快捷操作
- 原生列表重新渲染后的重复注入防护
...
- 确认扩展禁用后酒馆所有原生功能保持原状
- 以 100、1000 和 5000 条聊天索引检查分页、筛选与打开速度
...
4. 聊天可以按指定消息楼层通过原生接口生成新的单聊或群聊文件，无需修改酒馆源码
5. 分割必须默认非破坏、写后回读、摘要一致后才算完成
6. `/api/chats/recent` 可作为全局聊天枚举接口，欢迎页限制不需要通过修改酒馆源码解决
7. 全量索引必须显式触发并缓存，常规浏览只做最近记录增量刷新
