...
test('提示词查看器保持只读、可调整且不横向滚动', async () => {
    ...
    assert.match(css, /\.prompt-control-host\.prompt-control-input\s*\{[^}]*position:\s*fixed/s);
    ...
    assert.match(css, /@keyframes prompt-control-spin/);
    assert.match(css, /\.prompt-control-state \.fa-spin,[\s\S]*animation:\s*prompt-control-spin/);
    ...
    assert.match(ui, /form\.getBoundingClientRect\(\)/);
    assert.match(ui, /availableHeight/);
    assert.match(ui, /state\.input/);
    ...
    assert.match(template, /data-prompt-viewer-refresh/);
    assert.match(template, />提示词查看</);
    assert.doesNotMatch(template, /最终提示词/);
